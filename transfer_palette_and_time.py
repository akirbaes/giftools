...
def transfer_durations(source,images):
    im = Image.open(source)
    input_durations = list()
    try:
        while 1:
            input_durations.append(im.info.get("duration",0))
            im.seek(im.tell()+1)
    except EOFError:
        pass # end of sequence
    
    max_multiplier = int(len(images)/len(input_durations))
    output=list()
    output_durations = list()
    indexes = list()
    print("Max multiplier calc:", len(images),"/",len(input_durations),"=", max_multiplier)
    for index,duration in enumerate(input_durations):
        imindex = index*max_multiplier
        #print(imindex)
        ...
        if(imposs>30):    
            ...
            #Taper out
            indexes.append("Elongate")
            for i in range(max_multiplier):
                indexes.append(imindex+i)
                output.append(images[imindex+i])
            for i in range(max_multiplier):
                output_durations.append(int(round(duration/max_multiplier))) #Should be round /10 *10 maybe
        else:
            #Not enough frames
            ...
            for i in range(max_interpolation):
                try:
                    ...
                    output.append(images[finalindex])
                except Exception as e:
                    print("max_multiplier",max_multiplier)
                    print("Current original index:",index)
                    print("Current starting index for target",imindex)
                    print("max_interpolation",max_interpolation)
...
def create_gif_from_folder(source,foldername,outputname=None,palette=None):
    ...
    #print(durations)
    images,durations=transfer_durations(source,images)
    ...
    if __name__ == "__main__":
        ...
        if(len(sys.argv)>1):
            ...
            if(os.path.isdir(target)):
                ...
                create_gif_from_folder(source,target,name+"_EXT_%s.gif"%tname,palette=pal)
            else:
